...
\cite{tdt4258-2} might have a reference C implementation also, but I am not sure. :TODO.
...
This is because PIOC is made unavailable by \texttt{SW6} on the STK1002, see jumper settings TODO ref.
PIOB is therefore used for both buttons and LEDs.
The LEDs use some of the upper pins of PIOB - see \cite{compendium} page somewere (TODO where?) for a table.
...
For a more detailed explanation of the set-up of the buttons on the STK1000, see \cite{tdt4258-1}. Again, a C reference implementation might exists in \cite{4258-2} TODO: does it?
...
The installed linux defaults to muted audio settings, and the course guidelines \cite{compendium} TODO is it here? recommend using \texttt{alsamixer} to manually set the volume each time the board boots.
Because the authors like automation and tools, a script is provided that programatically sets the correct audio settings using the more low level \texttt{alsactl}.
